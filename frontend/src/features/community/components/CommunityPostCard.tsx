...
export function CommunityPostCard({ post, onAddComment, onBookmark, onLike, onShare }: CommunityPostCardProps) {
    ...
    <article className="surface-card overflow-hidden rounded">
      <div className="p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-slate-950">{post.author}</h3>
              <p className="text-sm text-slate-600">{post.role} · {post.createdAt}</p>
            </div>
          </div>
        ...
        <div className="mt-5">
          ...
          <p className="mt-3 text-sm leading-7 text-slate-800">{post.content}</p>
        </div>
      </div>
      ...
      <div className="px-5 py-4">
        ...
        <div className="grid grid-cols-4 gap-2 border-b border-slate-200 py-3">
          <ActionButton active={post.liked} icon={<Heart size={18} />} label="Thích" onClick={() => onLike(post.id)} />
          <ActionButton icon={<MessageCircle size={18} />} label="Bình luận" onClick={() => setExpandedComments(true)} />
          <ActionButton icon={<Share2 size={18} />} label="Chia sẻ" onClick={() => onShare(post.id)} />
          <ActionButton active={post.bookmarked} icon={<Bookmark size={18} />} label="Lưu" onClick={() => onBookmark(post.id)} />
        </div>
            ...
            {visibleComments.map((item) => (
              <div className="rounded bg-slate-50 px-4 py-3" key={item.id}>
                <div className="flex items-center justify-between gap-3">
                  <span className="text-sm font-semibold text-slate-950">{item.author}</span>
        ...
        <div className="mt-4 flex gap-3">
          <input
            ...
            value={comment}
          />
          <button className="grid h-11 w-11 place-items-center rounded bg-brand-900 text-white transition hover:bg-brand-700" onClick={submitComment} type="button">
            <Send size={17} />
          </button>
        </div>
      </div>
